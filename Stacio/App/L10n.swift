...
enum L10n {
    ...
    enum Workbench {
        ...
        static let sshSessionRequiredTooltip = "当前会话不支持此功能，仅 SSH 会话可用"
        ...
        static let sftpFilePaneOpened = "内置 SFTP 文件面板已打开"
    ...
    enum Import {
        ...
        static let invalidSessionData = "导入的会话配置无效。请确认文件为来源系统导出的原始配置后重试。"
        static let invalidPort = "导入文件中包含无效端口，请修正后重试。"
        static let databaseFailure = "保存导入的会话失败，请稍后重试。"
        static let referencedItemMissing = "导入过程中引用的会话或分组不存在，请刷新后重试。"
        static let credentialStorageFailure = "导入会话时无法安全保存凭据，请检查本机存储权限后重试。"
        static let genericFailure = "无法完成会话导入，请检查配置文件和本机访问权限后重试。"
        static let licenseUnavailableTooltip = "该功能模块无有效授权，请升级授权。"
        static let bastionVendorSelectionTitle = "选择堡垒机厂商"
        static let bastionVendorRetryAction = "重新识别"
        static func bastionVendorSelectionMessage(sourceName: String) -> String {
            "未能自动识别“\(sourceName)”的堡垒机厂商。请选择配置来源，Stacio 将按对应厂商规则重新识别。"
        }
        static func bastionVendorRetryFailed(_ vendorName: String) -> String {
            "已按“\(vendorName)”规则重新识别，但仍无法读取该配置。请确认文件为厂商导出的原始连接配置。"
        }
