...
UPSTREAM_MARKER='Deliberately not `Bundle.module`'
...
rm -f "$SOURCE_FILE.orig" "$SOURCE_FILE.rej"

if grep -Fq "$PATCH_MARKER" "$SOURCE_FILE" || grep -Fq "$UPSTREAM_MARKER" "$SOURCE_FILE"; then
  exit 0
fi
...
if ! /usr/bin/patch --batch --forward -p1 -d "$CHECKOUT_DIR" <"$PATCH_FILE"; then
  rm -f "$SOURCE_FILE.orig" "$SOURCE_FILE.rej"
  exit 1
fi
rm -f "$SOURCE_FILE.orig" "$SOURCE_FILE.rej"
