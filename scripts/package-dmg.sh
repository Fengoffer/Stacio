...
ALLOW_UNBRANDED_ARTIFACT="${STACIO_DMG_ALLOW_UNBRANDED_ARTIFACT:-0}"
...
if [[ "$SKIP_FINDER_LAYOUT" == "1" || "$SKIP_FINDER_LAYOUT" == "true" ]]; then
  if [[ "$ALLOW_UNBRANDED_ARTIFACT" != "1" && "$ALLOW_UNBRANDED_ARTIFACT" != "true" ]]; then
    fail "STACIO_DMG_SKIP_FINDER_LAYOUT is CI/test-only; set STACIO_DMG_ALLOW_UNBRANDED_ARTIFACT=1 to explicitly allow an unbranded DMG"
  fi
