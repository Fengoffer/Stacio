...
IMPORT_ICON_XSHELL="$APP_DIR/Contents/Resources/ImportSourceIcons/xshell.svg"
IMPORT_ICON_MOBAXTERM="$APP_DIR/Contents/Resources/ImportSourceIcons/mobaxterm.svg"
IMPORT_ICON_WINDTERM="$APP_DIR/Contents/Resources/ImportSourceIcons/windterm.svg"
IMPORT_ICON_SECURECRT="$APP_DIR/Contents/Resources/ImportSourceIcons/securecrt.svg"
...
test -s "$IMPORT_ICON_XSHELL"
test -s "$IMPORT_ICON_MOBAXTERM"
test -s "$IMPORT_ICON_WINDTERM"
test -s "$IMPORT_ICON_SECURECRT"
...
/usr/libexec/PlistBuddy -c "Print :NSAppTransportSecurity:NSAllowsArbitraryLoadsInWebContent" "$PLIST" | grep -Fq "true"
...
/usr/libexec/PlistBuddy -c "Print :StacioSparkleArchitecture" "$PLIST" | grep -Fq "arm64"
/usr/libexec/PlistBuddy -c "Print :SUFeedURL" "$PLIST" | grep -Fq "/stable/arm64/appcast.xml"
...
/usr/libexec/PlistBuddy -c "Print :SUScheduledCheckInterval" "$PLIST" | grep -Fq "86400"
